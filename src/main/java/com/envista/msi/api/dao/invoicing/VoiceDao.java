...
public class VoiceDao {
    ...
    public List<VoiceDto> getAllVoices(Long userId, int offset, int limit) {

        QueryParameter queryParameter = StoredProcedureParameter.with("P_USER_ID", userId)
                .and("P_VOICE_ID", 0L).and("P_PARENT_VOICE_ID", 0L).and("P_OFFSET", offset).and("P_PAGE_SIZE", limit).and("P_ACTION_TYPE", "getAllVoices");

        return persistentContext.findEntities("VoiceDto.getVoiceList", queryParameter);
    }

    public int getCountOfVoices(Long userId) {

        QueryParameter queryParameter = StoredProcedureParameter.with("P_USER_ID", userId)
                .and("P_VOICE_ID", 0L).and("P_PARENT_VOICE_ID", 0L).and("P_OFFSET", 0).and("P_PAGE_SIZE", 0).and("P_ACTION_TYPE", "countOfVoices");
    ...
    public List<VoiceDto> getParentVoiceNames(long voiceId) {
        ...
        QueryParameter queryParameter = StoredProcedureParameter.with("P_USER_ID", 0L)
                .and("P_VOICE_ID", 0L).and("P_PARENT_VOICE_ID", voiceId).and("P_OFFSET", 0).and("P_PAGE_SIZE", 0).and("P_ACTION_TYPE", "getAllParentVoices");
    ...
    public VoiceDto findByVoiceId(Long voiceId) {
        ...
        QueryParameter queryParameter = StoredProcedureParameter.with("P_USER_ID", 0L)
                .and("P_VOICE_ID", voiceId).and("P_PARENT_VOICE_ID", 0L).and("P_OFFSET", 0).and("P_PAGE_SIZE", 0).and("P_ACTION_TYPE", "findByVoiceId");
