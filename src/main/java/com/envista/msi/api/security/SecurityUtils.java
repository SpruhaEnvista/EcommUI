...
*/
    ...
    private ShpUserProfileTb getCurrentUserFromName() {
        ...
        try {
        	//user = userService.getUserByUserName(SecurityUtils.getCurrentUserLogin());
		} catch (Exception e) {
	...
	public static UserProfileBean getUserProfileBean() {
    	...
    	UserProfileBean user = null; // true == allow create
        HttpServletRequest httpSession = attr.getRequest();
        if (null != httpSession && null != httpSession.getSession()) {
        	user = (UserProfileBean) httpSession.getAttribute(USER_IN_SESSION);
        }
        return user;
    }
}
